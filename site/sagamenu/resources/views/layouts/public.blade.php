...
<head>
    ...
    @php
        ...
        $fontFormat = in_array(data_get($appearance, 'custom_font_format'), ['woff', 'woff2'], true)
            ...
            : 'woff2';
        $fontAllowlist = ['Instrument Sans', 'Inter', 'Georgia'];
        $headingFont = in_array(data_get($appearance, 'heading_font'), $fontAllowlist, true) ? data_get($appearance, 'heading_font') : 'Instrument Sans';
        $bodyFont = in_array(data_get($appearance, 'body_font'), $fontAllowlist, true) ? data_get($appearance, 'body_font') : 'Instrument Sans';
    @endphp
    <style>
