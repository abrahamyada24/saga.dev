...
@section('content')
    ...
    <h2 id="auth-title">Aktifkan akunmu</h2>
    ...
    <form method="post" action="{{ route('saga-platform.verify') }}">
        @csrf
        <div class="field"><label for="verification_token">Token verifikasi</label><input id="verification_token" name="verification_token" value="{{ old('verification_token', $token) }}" autocomplete="one-time-code" spellcheck="false" required @error('verification_token') aria-invalid="true" aria-describedby="form-errors" @enderror></div>
