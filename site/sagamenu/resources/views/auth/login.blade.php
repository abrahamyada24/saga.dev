...
@section('content')
    ...
    <h2 id="auth-title">Masuk ke Saga Menu</h2>
    <p class="intro">Gunakan akun pusat yang sudah terverifikasi.</p>
    <form method="post" action="{{ route('saga-platform.login') }}">
        @csrf
        <div class="field"><label for="email">Email</label><input id="email" type="email" name="email" value="{{ old('email') }}" autocomplete="email" required autofocus @error('email') aria-invalid="true" aria-describedby="form-errors" @enderror></div>
